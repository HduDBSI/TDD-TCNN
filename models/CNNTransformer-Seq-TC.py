...
class Config(object):
    ...
    def __init__(self, args, dataset, embedding):
        self.model_name = 'CNNTransformer-Seq-TC'
        path = "/data/td_detection/"
        manual_dataset_path=path+dataset + '-' + args.dataset if hasattr(args, 'dataset') and args.dataset else ""
        ...
        self.class_list = manual_dataset_path+ '/data/class.txt' if hasattr(args, 'dataset') and args.dataset else [x.strip() for x in open(
        ...
        # self.num_layers = args.num_layers if hasattr(args, 'num_layers') and args.num_layers else 3
        self.num_head = args.num_head if hasattr(args, 'num_head') and args.num_head else 4
        ...
        self.filter_sizes = args.filter_sizes if hasattr(args, 'filter_sizes') and args.filter_sizes else (1,2,3)
        self.num_filters = args.num_filters if hasattr(args, 'num_filters') and args.num_filters else 128

        self.device = torch.device('cuda:'+args.device) if hasattr(args, 'device') and args.device else torch.device('cuda:2' if torch.cuda.is_available() else 'cpu')
...
class TransformerModel(nn.Module):
    def __init__(self, config):
        ...
        # self.hidden_dim = config.embed
        # self.dropout = config.dropout
        # self.pos_encoder = Positional_Encoding(self.hidden_dim, self.dropout)
        self.postion_embedding = Positional_Encoding(config.embed, config.pad_size, config.dropout, config.device)

        encoder_layers = TransformerEncoderLayer(config.dim_model,config.num_head,config.dim_feedforward,config.dropout)

        self.transformer_encoder = TransformerEncoder(encoder_layers,config.num_encoder)
...
class Model(nn.Module):
    ...
    def forward(self, x):
        # 首先将输入传递给TransformerModel
        x = self.transformer_model(x)
        # 将TransformerModel的输出传递给TextCNN
        x = self.text_cnn(x)
        return x
